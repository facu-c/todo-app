...
pub fn create_task(tasks: &mut Vec<ConfigFile>, id_task: u64) {
    ...
    tasks.push(ConfigFile {
        id_task,
        name_task: input_name_task.trim().to_string(),
        desc_task: input_desc_task.trim().to_string(),
        done: false,
    });
...
pub fn list_task() {
    ...
    for element in tasks {
        println!(
            "{}: {} - {} - {}",
            element.id_task, element.name_task, element.desc_task, element.done
        );
        println!("---------------");
    }
}
